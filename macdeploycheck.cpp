...
int main(int argc, char **argv) {
  ...
  qInfo() << "Running macdeploycheck";

  if (argc < 1) {
    qCritical() << "Usage: macdeploycheck <bundledir>";
    return 1;
  }
  ...
  while (iter.hasNext()) {
    ...
    if (otool.exitStatus() != QProcess::NormalExit || otool.exitCode() != 0) {
      qCritical() << "otool failed for" << filepath << ":" << otool.readAllStandardError();
      success = false;
      continue;
    }
    ...
    if (output_lines.size() < 2) {
      qCritical() << "Could not parse otool output:" << output;
      success = false;
      continue;
    }
    ...
    if (first_line == filepath) {
      ...
      qCritical() << "First line" << first_line << "does not match" << filepath;
      success = false;
    }
      ...
      if (match.hasMatch()) {
        ...
        if (QFileInfo(library).fileName() == QFileInfo(filepath).fileName()) {  // It's this.
          ...
          if (!QFile::exists(real_path)) {
            qCritical() << real_path << "does not exist for" << filepath;
            success = false;
          }
        }
          ...
          qCritical() << "File" << filepath << "points to" << library;
          success = false;
        }
      }
      else {
        qCritical() << "Could not parse otool output line:" << output_line;
        success = false;
      }
    }
  }
